...
# Configure logging
logging.basicConfig(filename='process_export_dauso_scb.log',
...
def update_status_and_url(record_id, outbox_id, file_path, file_name):
    ...
    update_query = """
        UPDATE app_fd_excel_dau_so_scb_vgw 
    ...
    update_query = """
           UPDATE outbox_excel_dau_so_scb 
...
def main_thread():
    ...
    try:
        ...
        if not group_name:  # Kiểm tra nếu group_name là None hoặc chuỗi rỗng
            group_names = ['Sacombank-Caller-Status', 'Sacombank-Backup-Caller-Status']
            filtered_data = process_multiple_groups(r, group_names, c_status)
        else:
            logging.info(f"Processing single group: {group_name}")
            filtered_data = process_group(r, group_name, c_status)

        output_dir = f'/home/Export_dau_so_scb/{record_id}/'
        os.makedirs(output_dir, exist_ok=True)

        # Lấy thời gian hiện tại và định dạng tên file
        current_time = datetime.now().strftime('%d-%m-%Y %H:%M:%S')
        file_name = f'export_dauso_{current_time}.xlsx'
        output_file = os.path.join(output_dir, f'export_dauso_scb_{current_time}.xlsx')
        relative_file_path = f'{record_id}/export_dauso_scb_{current_time}.xlsx'
